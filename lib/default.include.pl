...
sub default_ua_init() {
	...
	$global_useragent  = LWP::UserAgent->new('agent' => "Logix PBX");
...
sub do_pbx_login_check() {
	$check_url = "https://$app{pbx_host}/app/edit/index.php?dir=xml";
...
sub do_pbx_login() {
	...
	for (1..3) {
		...
		$loginurl = "https://$domain/core/user_settings/user_dashboard.php";
...
sub post_data () {
	...
	$url = "https://$app{pbx_host}" . $data{urlpath};
	#warn $url;
	#warn @{$data{data}};
	if (@{$data{data}}) {
