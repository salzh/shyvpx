...
sub addconference () {
	local %params = (
		...
		conference_order => {type => 'string', maxlen => 5, notnull => 0, default => '333'},
...
sub editconference () {
    local %params = (
        conference_name => {type => 'string', maxlen => 50, notnull => 0, default => ''},
		conference_extension => {type => 'string', maxlen => 50, notnull => 0, default => ''},
		conference_pin_number => {type => 'string', maxlen => 10, notnull => 0, default => ""},
		conference_profile => {type => 'enum:default,wideband,ultrawideband,cdquality',
                               maxlen => 255, notnull => 0, default => "default"},
        conference_flags => {type => 'string', maxlen => 50, notnull => 0, default => ''},
        ...
        conference_uuid   => {type => 'string', maxlen => 50, notnull => 1, default => ''},
		user_uuid => {type => 'string', maxlen => 100, notnull => 0, default => ''}
    );
	...
	%hash = &database_select_as_hash(
					...
					from
						v_conferences
					where
						conference_uuid='$post_add{conference_uuid}'",
					'uuid');
						
	if (!$hash{1}{uuid}) {
		$response{stat}		= "fail";
		$response{message}	= "Conference uuid not Exist!";
	}
	...
	if (!$hash{1}{uuid}) {
		$response{stat}		= "fail";
		$response{message}	= "dialplan_uuid not Exist!";
	}
...
sub getconference () {
    ...
    if ($hash{1}{conference_uuid}) {
        ...
        #Added by Ashvini 11-11-2021
        ######## Get all user_uuid ############
          @userid = ();
          %user = &database_select_as_hash(
                    "select
                         v_conference_users.user_uuid,v_users.username
                    from
                         v_users left join v_conference_users
                    on
                         v_users.user_uuid=v_conference_users.user_uuid
                    where
                        v_conference_users.conference_uuid='$conference_uuid'",
                    'username');
