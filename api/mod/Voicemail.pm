...
sub addvoicemail () {
	...
	%hash_1 = &database_select_as_hash("select
											1,voicemail_uuid
										from
											v_voicemails
										where
	...
	if($hash_1{1}{uuid}){
	   $response{stat}		= "fail";
	   $response{message}	= "$post_add{voicemail_uuid} Already Exist!";
	} 
...
sub editvoicemail () {
  local %params = (
        ...
        voicemail_enabled => {type => 'bool', maxlen => 10, notnull => 0, default => 'true'},
        voicemail_description => {type => 'string', maxlen => 255, notnull => 0, default =>'false'},
		voicemail_uuid_copy => {type => 'string', maxlen => 50, notnull => 0, default => ''},
        referer_path => {type => 'string', maxlen => 255, notnull => 0, default => '/app/voicemails/voicemails.php'}
    );
    ...
    %hash_1 = &database_select_as_hash("select
										...
										from
											v_voicemails
										where
											voicemail_uuid='$post_add{voicemail_uuid}'
											and domain_uuid='$domain{uuid}'",
										'uuid');
	
	if(!$hash_1{1}{uuid}){
	   $response{stat}		= "fail";
	   $response{message}	= "voicemail_uuid Not Exist!";
	} 
