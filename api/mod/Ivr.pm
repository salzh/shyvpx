...
';
	...
	local %params = (
		...
		#Added by Hemant 08-10-2021
		ivr_menu_tts_voice => {type => 'string', maxlen => 255, notnull => 0, default => "rms"},
		ivr_menu_tts_engine => {type => 'string', maxlen => 255, notnull => 0, default => "flite"},
		#End
...
ivr_menu_greet_long:voicemail/vm-play_next_message.wav
ivr_menu_greet_short:voicemail/vm-hello.wav
ivr_menu_options[0][ivr_menu_option_uuid]:
...
';
    ...
    local %params = (
        ...
        ivr_menu_name => {type => 'string', maxlen => 50, notnull => 0, default => ''},
		ivr_menu_extension => {type => 'string', maxlen => 50, notnull => 0, default => ''},
		ivr_menu_greet_long => {type => 'string', maxlen => 255, notnull => 0, default => ""},
		ivr_menu_greet_short => {type => 'string', maxlen => 255, notnull => 0, default => ""},
	...
	if ($ivr_ext_edt{1}){
		...
		if ($response{stat} ne 'fail') {		
			...
			for (0..99) {
				last unless $form{"ivr_menu_options[$_][ivr_menu_option_param]"};
				#Added BY Hemant Chaudhari 08-10-2021
				$post_add{"ivr_menu_options[$_][ivr_menu_option_uuid]"}
						= &database_clean_string($form{"ivr_menu_options[$_][ivr_menu_option_uuid]"});
				#end
				$post_add{"ivr_menu_options[$_][ivr_menu_option_digits]"}
						= &database_clean_string($form{"ivr_menu_options[$_][ivr_menu_option_digits]"});
				$post_add{"ivr_menu_options[$_][ivr_menu_option_param]"}
			...
			if (!$uuid) {
				$response{stat}		= "fail";
				$response{message}	= "IVR UUID not Exist!";
			} else {
				$response{stat}					= "ok";
				$response{data}{ivr_menu_uuid}	= $uuid;
			}     
